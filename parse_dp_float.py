#!/usr/bin/env python
"""
Take 16-digit-hex-strings or float values and print detailed
information about the double precision float they represent
"""
import math
import struct
import sys
...
def hex2float(hexstring):
    """ Take a string of 16 hex digits and convert it to the double precision
        float represented by it .
    """
    return struct.unpack('!d', hexstring.decode('hex'))[0]

def float2hex(float_val):
    """ Take a float and return 16 hex digits representing it in double
        precision.
    """
    try:
        return binascii.hexlify(struct.pack('!d', float_val))
...
def main(argv=None):
    ...
    hex_pattern = re.compile(r'^[0-9abcdefABCEDF]{16}$')
    if argv is None:
        argv = sys.argv
        ...
        arg_parser.add_option(
          ...
          help="Format to use when printing floats (default '%.15e')")
        ...
        for value in args[1:]:
            ...
            if not hex_pattern.match(value):
                try:
                    ...
                    "is neither a valid 16-digit hex string "
                    "nor a valid double precision float" )
